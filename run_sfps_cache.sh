...
sfps_version=$sfps_ver
if [ -z "${sfps_version}" ]
then
    sfps_version=SFPS-0.8.5.1
fi
...
echo "install_sfps: ${install_sfps}"
echo "USE_SFPS: ${USE_SFPS}"
echo "----------------------------------------------------------------"

# sfps_install
if [ "${install_sfps}" = "true" ]
then
    sh ${train_workdir}/sfps_install.sh $USE_SFPS $lib_ps $sfps_version
fi
