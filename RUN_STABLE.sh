...
feature_config=${algorithm_path}/featureConfig
if [ ! -e ${code_path} ]
then
...
if [ -n "${model_file}" ] && [ -n "${only_update_config}" ]
then
    echo "Update only the config file."
    cp $model_file/* $model_path
    cp -r -f $feature_config/* $model_path
    exit 0
fi
...
config_path=${algorithm_path}/config_task.py
